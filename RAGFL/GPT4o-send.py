...
def faultlocalization(prompt_location, code_location, res_path, db_path, files_loc, experiment_model, faultdata):
    ...
    # prompt = build_newoneshot(oneshot_prompt, code_location, causes, fix)
...
def test_DebugBench(prompt_location, experiment_index, experiment_model, rangeIndex):
    ...
    for version_str in DebugBench_Filter_Data:
        ...
        problem_location = os.path.join(root_path, version_str, "problemDes/problem.txt")
        ...
        isok = faultlocalization(problem_location, prompt_location, code_location, res_path, db_path, files_loc, experiment_model, faultline_indexes)
...
def run_all(prompt_location):
    # 批量跑实验
    experiment_model = "claude-3-haiku-20240307"
    for i in [2]:
